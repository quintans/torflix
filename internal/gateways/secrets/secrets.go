...
func (s *Secrets) GetOpenSubtitles() (app.OpenSubtitlesSecret, error) {
	...
	if err != nil {
		...
		return app.OpenSubtitlesSecret{}, faults.Errorf("could not get OpenSubtitles: %w", err)
	}

	var secret app.OpenSubtitlesSecret
	err = json.Unmarshal([]byte(data), &secret)
	if err != nil {
		return app.OpenSubtitlesSecret{}, faults.Errorf("could not unmarshal OpenSubtitles: %w", err)
	}
...
func (s *Secrets) SetOpenSubtitles(value app.OpenSubtitlesSecret) error {
	...
	if err != nil {
		return faults.Errorf("could not marshal OpenSubtitles: %w", err)
	}
	err = keyring.Set("torflix", "opensubtitles", string(data))
	if err != nil {
		return faults.Errorf("could not save OpenSubtitles: %w", err)
	}
